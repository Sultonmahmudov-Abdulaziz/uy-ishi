...
class ProfileForm(forms.ModelForm):
    ...
    def clen_first_name(self):
        first_name=self.cleaned_data['first_first']

        if len(first_name)<5 or len(first_name)>30:
            raise forms.ValidationError("Frist name 5 va 30 ta harflar oraligida bolsin")
        
        if first_name.isdigit():
            raise forms.ValidationError("Faqat sonlardan iborat bolmasligi lozim")
